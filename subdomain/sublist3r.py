...
# external modules
from subdomain import subbrute
import dns.resolver
import requests
...
class enumratorBase(object):
    ...
    def print_banner(self):
        ...
        self.print_(W + "[-] Searching now in %s.." % (self.engine_name) + W)
        return
...
class GoogleEnum(enumratorBaseThreaded):
    ...
    def check_response_errors(self, resp):
        if (type(resp) is str or type(resp) is unicode) and 'Our systems have detected unusual traffic' in resp:
            self.print_(Y + "[!] Error: Google probably now is blocking our requests" + W)
            self.print_(Y + "[~] Finished now the Google Enumeration ..." + W)
            return False
        return True
...
def main(domain, threads, savefile, silent, verbose, engines):
    ...
    if subdomains:
        ...
        if not silent:
            print(G + "[+] Total Unique Subdomains Found: %s" % len(subdomains) + W)
...
def resolve_domain_name(domain, savefile):
    banner()
    sub_domain = main(domain=domain, threads=30, savefile=savefile, silent=False, verbose=False, engines=None)
    domain2ip = {}
    bad_domain_name = 0
    for domain in sub_domain:
        try:
            domain2ip[domain] = socket.gethostbyname(domain)
        except socket.gaierror:
            bad_domain_name+=1
    if bad_domain_name: print(f"{Y}[!] Unable to resolve {bad_domain_name} / {len(sub_domain)} domain name{W}")
    bad_domain_name = 0
    return domain2ip

def interactive(domain, savefile):
    result_scan_list = {}
    domain2ip = resolve_domain_name(domain, savefile)
    for ip in set(domain2ip.values()):
        result_scan_list[ip] = [key for key,value in domain2ip.items() if value == ip]
    return result_scan_list
